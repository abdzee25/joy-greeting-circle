...
async function loadCache(): Promise<Dataset> {
  const [dRes, pRes, sRes] = await Promise.all([
    fetch(URLS.description),
    fetch(URLS.precaution),
    fetch(URLS.severity),
  ]);
  if (!dRes.ok || !pRes.ok || !sRes.ok) throw new Error("Failed to fetch dataset CSVs");
  const [dTxt, pTxt, sTxt] = await Promise.all([dRes.text(), pRes.text(), sRes.text()]);

  const descriptions = new Map<string, string>();
  for (const row of parseCsv(dTxt).slice(1)) {
    const [disease, desc] = row;
    if (disease) descriptions.set(norm(disease), desc ?? "");
  }
  const precautions = new Map<string, string[]>();
  for (const row of parseCsv(pTxt).slice(1)) {
    const [disease, p1, p2, p3, p4] = row;
    if (disease) {
      precautions.set(norm(disease), [p1, p2, p3, p4].map((x) => (x ?? "").trim()).filter(Boolean));
    }
  }
  const severity = new Map<string, number>();
  for (const row of parseCsv(sTxt).slice(1)) {
    const [sym, w] = row;
    if (sym) {
      const n = Number(w);
...
export function lookupDescription(c: Dataset, disease: string): string | null {
...
export function lookupPrecautions(c: Dataset, disease: string): string[] | null {
  const key = norm(disease);
